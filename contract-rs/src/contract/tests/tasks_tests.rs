...
fn test_deposits_assigned_correctly_after_task_dispute_resolution() {
    ...
    // check balances
    let expected_contractor_deposit = dec!(745); // 800(1000 * 80 / 100 - resolution percentage for contractor) - 5(1000 * 0.005 contractor fee) - 50(1000 * 0.05 dispute resolution fee)
    let expected_owner_deposit = dec!(200); // 200(1000 - 1000 * 80 / 100 - resolution percentage for owner)
